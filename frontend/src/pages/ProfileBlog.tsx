            ...
            <div className="lg:w-1/2 lg:max-w-1/2 w-full mx-2 lg:mx-0">
                {
                    ...
                    : blogs.length>0 ? (blogs.map((blog, index) => <BlogCard editable={true} key={index} id={blog.id} authername={blog.auther.name || "Anonymous"} title={blog.title} content={blog.content} publishedDate={blog.publishedOn} />)) : <NoLogsFound />
                }
            </div>
        </div>
    </>
